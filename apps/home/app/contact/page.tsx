...
function TextInput({
  ...
  return (
    <div className="group relative z-0 transition-all focus-within:z-10">
      ...
      <label
        htmlFor={id}
        className="pointer-events-none absolute left-6 top-1/2 -mt-3 origin-left text-base/6 text-neutral-500 transition-all duration-200 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:font-semibold peer-focus:text-black peer-[:not(:placeholder-shown)]:-translate-y-4 peer-[:not(:placeholder-shown)]:scale-75 peer-[:not(:placeholder-shown)]:font-semibold peer-[:not(:placeholder-shown)]:text-black"
      >
        {label}
      </label>
    </div>
...
function ContactForm() {
  return (
    <FadeIn className="lg:order-last">
      <form>
        <h2 className="font-display text-base font-semibold text-black">Work inquiries</h2>
...
function ContactDetails() {
  return (
    <FadeIn>
      <h2 className="font-display text-base font-semibold text-black">Our offices</h2>
      ...
      <Border className="mt-16 pt-16">
        <h2 className="font-display text-base font-semibold text-black">Email us</h2>
        <dl className="mt-6 grid grid-cols-1 gap-8 text-sm sm:grid-cols-2">
          {[
            ['Careers', 'careers@watheia.com'],
            ['Press', 'press@watheia.com'],
          ].map(([label, email]) => (
            <div key={email}>
              <dt className="font-semibold text-black">{label}</dt>
              <dd>
                <Link href={`mailto:${email}`} className="text-neutral-600 hover:text-black">
                  {email}
                </Link>
              </dd>
            </div>
      ...
      <Border className="mt-16 pt-16">
        <h2 className="font-display text-base font-semibold text-black">Follow us</h2>
