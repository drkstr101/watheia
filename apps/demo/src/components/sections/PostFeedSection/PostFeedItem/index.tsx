  ...
  return (
    <Link
      ...
      className={classNames(
        ...
        post.styles?.self?.padding
          ? mapStyles({ padding: post.styles?.self?.padding })
          : undefined,
        post.styles?.self?.borderWidth &&
          post.styles?.self?.borderWidth !== 0 &&
          post.styles?.self?.borderStyle !== 'none'
          ? mapStyles({
          ...
          : undefined,
        post.styles?.self?.borderRadius
          ? mapStyles({ borderRadius: post.styles?.self?.borderRadius })
          : undefined,
        post.styles?.self?.textAlign
          ? mapStyles({ textAlign: post.styles?.self?.textAlign })
          : undefined,
        'overflow-hidden',
        'transition',
        'duration-200',
    ...
    >
      <div
        className={classNames(
          'w-full',
          'flex',
          mapFlexDirectionStyles(flexDirection, hasThumbnail),
          'gap-6'
        )}
      >
        {hasThumbnail && (
          <ImageBlock
            ...
            className={classNames({
              'xs:w-[50%] xs:shrink-0':
                hasBigThumbnail && (flexDirection === 'row' || flexDirection === 'row-reversed'),
              'xs:w-[28.4%] xs:shrink-0':
                !hasBigThumbnail && (flexDirection === 'row' || flexDirection === 'row-reversed'),
            })}
        ...
        <div
          className={classNames('w-full', {
            'xs:grow':
              hasThumbnail && (flexDirection === 'row' || flexDirection === 'row-reversed'),
          })}
        >
          {hasSectionTitle ? (
