  ...
  return (
    <Annotated content={props}>
      {url ? (
        <Link
          href={url}
          className="hover:bottom-shadow-6 relative inline-flex rounded-full border-2 border-current px-5 py-4 text-lg leading-tight no-underline transition hover:-translate-y-1.5"
        >
          {label}
        </Link>
      ) : (
        <div className="inline-flex rounded-full border-2 border-current px-5 py-4 text-lg leading-tight">
          {label}
        </div>
      )}
    </Annotated>
