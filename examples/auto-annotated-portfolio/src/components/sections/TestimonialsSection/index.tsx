...
export default function TestimonialsSection(props) {
  ...
  return (
    <Section elementId={elementId} colors={colors} styles={styles.self}>
      ...
      <TestimonialVariants
        {...rest}
        className={title || subtitle ? 'mt-16 sm:mt-20' : undefined}
      />
    </Section>
  );
}
...
function TestimonialsVariantA(props) {
  ...
  return (
    <div className={classNames('grid gap-y-16 md:grid-cols-2 md:gap-y-20', className)}>
      ...
      {testimonials.map((testimonial, index) => (
        ...
        >
          ...
          <div className="grow">
            ...
            {(testimonial.name || testimonial.title) && (
              <footer className="mt-8 flex flex-wrap items-center">
                {(testimonial.name || testimonial.title) && (
                  <div className="grow text-lg">
                    {testimonial.name && <div>{testimonial.name}</div>}
                    {testimonial.title && <div>{testimonial.title}</div>}
