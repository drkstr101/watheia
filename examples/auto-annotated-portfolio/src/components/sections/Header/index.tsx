...
export default function Header(props) {
  ...
  return (
    ...
    >
      <div
        className={classNames({
          'mx-auto max-w-7xl xl:border-x xl:border-current': headerWidth === 'narrow',
  ...
  return (
    <div className="relative flex items-stretch">
      ...
      {primaryLinks.length > 0 && (
        <ul
          className={classNames(
            'hidden divide-x divide-current border-l border-current lg:flex',
            {
              'ml-auto': primaryLinks.length === 0,
            },
          )}
  ...
  return (
    <div className="ml-auto lg:hidden">
      <button
        ...
        className="h-10 min-h-full border-l border-current p-4 text-lg focus:outline-hidden"
        onClick={() => setIsMenuOpen(true)}
      >
        <MenuIcon className="w-icon h-icon fill-current" />
      </button>
      <div
        className={classNames(
          'bg-main fixed inset-0 z-20 overflow-y-auto',
        ...
        <div className="flex min-h-full flex-col">
          <div className="flex items-stretch justify-between border-b border-current">
            ...
            <div className="border-l border-current">
              ...
              >
                <CloseIcon className="w-icon h-icon fill-current" />
              </button>
            </div>
          </div>
          {(primaryLinks.length > 0 || socialLinks.length > 0) && (
            <div className="flex grow flex-col items-center justify-center space-y-12 px-4 py-20">
              {primaryLinks.length > 0 && (
              ...
              {socialLinks.length > 0 && (
                <ul className="flex flex-wrap justify-center divide-x divide-current border border-current">
...
function SiteLogoLink({ title, isTitleVisible, logo }) {
  ...
  return (
    <div className="flex items-center border-r border-current">
      <Link href="/" className="link-fill flex h-full items-center gap-2 p-4">
        {logo && <ImageBlock {...logo} className="max-h-12" />}
        {title && isTitleVisible && (
          <span className="text-base tracking-widest uppercase">{title}</span>
        )}
      </Link>
    </div>
  );
  ...
  return links.map((link, index) => (
    ...
    >
      <HeaderLink
        {...link}
        className={classNames(
          inMobileMenu ? 'bottom-shadow-1 hover:bottom-shadow-5 text-xl' : 'link-fill p-4',
        )}
      />
    </li>
  ));
...
function ListOfSocialLinks({ links, inMobileMenu = false }) {
  return links.map((link, index) => (
    <li key={index} className="inline-flex items-stretch">
      <Social
        {...link}
        className={classNames('link-fill text-lg', inMobileMenu ? 'p-5' : 'p-4')}
      />
    </li>
  ));
}
