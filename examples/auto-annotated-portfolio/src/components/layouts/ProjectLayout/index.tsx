  ...
  return (
    <BaseLayout {...props}>
      <article className="px-4 py-14 lg:py-20">
        ...
        {description && (
          <div className="mx-auto mb-10 max-w-3xl text-lg uppercase sm:mb-14 sm:text-xl">
            {description}
          </div>
        )}
        {media && (
      ...
      {(prevProject || nextProject) && (
        <nav className="mt-12 mb-20 px-4">
          <div className="mx-auto grid max-w-5xl gap-x-6 gap-y-12 sm:grid-cols-2 lg:gap-x-8">
            {prevProject && <ProjectNavItem project={prevProject} className={undefined} />}
            {nextProject && (
              <ProjectNavItem project={nextProject} className="sm:col-start-2 sm:items-end" />
            )}
          </div>
        </nav>
      )}
...
function ProjectNavItem({ project, className }) {
  return (
    <Annotated content={project}>
      <Link
        className={classNames('group flex flex-col items-start gap-6', className)}
        href={project}
      >
        ...
        <span className="bottom-shadow-1 group-hover:bottom-shadow-5 text-lg leading-tight uppercase transition">
          {project.title}
        </span>
      </Link>
    </Annotated>
