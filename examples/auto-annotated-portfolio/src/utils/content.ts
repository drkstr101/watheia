...
function resolveReferences(
  content: types.ContentObject,
  fileToContent: Record<string, types.ContentObject>,
) {
...
function annotateContentObject(o: any, prefix = '', depth = 0) {
  ...
  if (depth === 0) {
    if (o.__metadata?.id) {
      ...
      if (logAnnotations)
        console.log(
          '[annotateContentObject] added object ID:',
          depthPrefix,
          o[types.objectIdAttr],
        );
    } else {
    ...
    if (logAnnotations)
      console.log(
        '[annotateContentObject] added field path:',
        depthPrefix,
        o[types.fieldPathAttr],
      );
  }
