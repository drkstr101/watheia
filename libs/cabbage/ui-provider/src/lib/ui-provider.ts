...
import { withPersonalization } from './util/ninetailed/with-personalization';
...
export function getComponent(key: string): ComponentType {
  ...
  return personalizedComponents[key]
    ? withPersonalization(component, personalizedComponents[key])
    : component;
}
...
const components = {
  ...
  Page: dynamic(() => import('./layouts/page-layout')),
  Article: dynamic(() => import('./layouts/article-layout')),
