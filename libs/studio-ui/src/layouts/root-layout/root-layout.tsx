    ...
    <Container>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-x-8">
          <Button href="/contact" color={invert ? 'white' : 'black'}>
            Contact us
          </Button>
          <button
            ref={toggleRef}
            ...
            <Icon
              className={clsx(
                ...
                invert
                  ...
                  : 'fill-black group-hover:fill-neutral-700'
              )}
            />
          </button>
        </div>
...
function NavigationItem({ href, children }: { href: string; children: ReactNode }) {
  return (
    <Link
      href={href}
      className="group relative isolate -mx-6 bg-black px-6 py-10 even:mt-px sm:mx-0 sm:px-0 sm:py-16 sm:odd:pr-16 sm:even:mt-0 sm:even:border-l sm:even:border-neutral-800 sm:even:pl-16"
    >
      {children}
    ...
    <MotionConfig transition={shouldReduceMotion ? { duration: 0 } : undefined}>
      <header>
        ...
        >
          <motion.div layout className="bg-neutral-800">
            ...
            <Navigation />
            <div className="relative bg-black before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-neutral-800">
              <Container>
      ...
      >
        <motion.div layout className="relative isolate flex w-full flex-col pt-9">
          <GridPattern
            className="absolute inset-x-0 -top-14 -z-10 h-[1000px] w-full fill-white stroke-black/5 [mask-image:linear-gradient(to_bottom_left,white_40%,transparent_50%)]"
            yOffset={-96}
            interactive
          />
